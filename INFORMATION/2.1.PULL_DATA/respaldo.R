...
#
pwalk(list(paste0("table", names(sql_tables)),
           paste0("id", names(sql_tables)),
           names(sql_tables)),
      function(elm, hlm, tn){
        
        
        
        observeEvent(eval(parse(text = paste0("input$",elm))),{
          
          updateSelectInput(session, inputId = eval(parse(text = paste0("input$",hlm))), 
                            choices = sql_tables[[tn]])
        })
      })
#